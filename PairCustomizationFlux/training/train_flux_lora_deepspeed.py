...
from modules.src.flux.sampling import denoise, get_noise, get_schedule, prepare, unpack, prepare_prompt, prepare_img
...
from modules.src.flux.xflux_pipeline import XFluxSampler
...
def set_lora_weights_dit(dit, mode):
...
def change_weights_requires_grad(dit, requires_grad, pattern_match_name):
    if type(pattern_match_name) == str:
        pattern_match_name = [pattern_match_name]
    for name, parameter in dit.named_parameters():
        if all([pattern in name for pattern in pattern_match_name]):
...
def main():
    ...
    # change_weights_requires_grad(dit, False, "style")
    ...
    content_requires_grad_parameters = [p for n, p in dit.named_parameters() if 'content' in n and p.requires_grad]
    for n, p in dit.named_parameters():
        if 'content' in n and p.requires_grad:
            print(n)
    
    seed_everything(42)
    optimizer_content = optimizer_cls(
        content_requires_grad_parameters,
    ...
    if wandb is not None:
        wandb.init(
            entity="cmu-gil",
            name="LoRA Flux intiial experiment",
            group="LoRA Flux",
            project='LoRA-Flux',
        )
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(dit):
                ...
                with torch.no_grad():
                    ...
                    inp = prepare_img(img=x_1)
                    inp.update(content_txt_inps[content_prompt[0]]) # batch size is 1 for our training
                ...
                for n, p in dit.named_parameters():
                    if "single_blocks.1.processor.proj_lora_content.down.weight" in n:
                        # logger.info(f"p before updates: {p}", main_process_only=False)
                        logger.info(f"p grad before updates: {p.grad}, p requires grad: {p.requires_grad}", main_process_only=False)
                if accelerator.sync_gradients:
                    accelerator.clip_grad_norm_(dit.parameters(), args.max_grad_norm)
                optimizer.step()
                for n, p in dit.named_parameters():
                    if "single_blocks.1.processor.proj_lora_content.down.weight" in n:
                        # logger.info(f"p after updates: {p}", main_process_only=False)
                        logger.info(f"p grad after updates: {p.grad}, p requires grad: {p.requires_grad}", main_process_only=False)
                exit(0)
                lr_scheduler.step()
                optimizer.zero_grad()
            ...
            if accelerator.sync_gradients:
                ...
                if not args.disable_sampling and global_step % args.sample_every == 0:
                    if accelerator.is_main_process:
                        ...
                        sampler = XFluxSamplerPairCustomization(clip=None, t5=None, ae=vae, model=dit, device=accelerator.device, offload=False, content_prompt_inps=content_text_inputs_copy, style_prompt_inps={}) # 
                        images = []
                        for i, prompt in enumerate(args.sample_prompts):
                            ...
                            os.makedirs("LoRA-intermediate-flux-samples", exist_ok=True)
                            result.save(f"LoRA-intermediate-flux-samples/{global_step}_prompt_{i}_res.png")
