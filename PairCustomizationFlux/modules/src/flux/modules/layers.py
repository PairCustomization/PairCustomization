...
class DoubleStreamPairedBlockLoraProcessor(nn.Module):
    def __init__(self, dim: int, rank=4, network_alpha=None, lora_weight=1):
        super().__init__()
        self.qkv_lora1_style = LoRALinearLayer(dim, dim * 3, rank, network_alpha)
        self.proj_lora1_style = LoRALinearLayer(dim, dim, rank, network_alpha)
        self.qkv_lora2_style = LoRALinearLayer(dim, dim * 3, rank, network_alpha)
        self.proj_lora2_style = LoRALinearLayer(dim, dim, rank, network_alpha)

        self.qkv_lora1_content = LoRALinearLayer(dim, dim * 3, rank, network_alpha)
        self.proj_lora1_content = LoRALinearLayer(dim, dim, rank, network_alpha)
        self.qkv_lora2_content = LoRALinearLayer(dim, dim * 3, rank, network_alpha)
        self.proj_lora2_content = LoRALinearLayer(dim, dim, rank, network_alpha)

        self.mode = "both"
        self.lora_weight = lora_weight

    def change_mode(self, mode: str):
        assert mode in ["both", "style", "content", "none"]
        self.mode = mode

    def get_lora_params(self, mode: str):
        if self.mode == "style":
            return self.qkv_lora1_style, self.qkv_lora2_style, self.proj_lora1_style, self.proj_lora2_style
        elif self.mode == "content":
            return self.qkv_lora1_content, self.qkv_lora2_content, self.proj_lora1_content, self.proj_lora2_content
        elif self.mode == "none":
            return lambda x: torch.tensor(0.0).to(x.dtype).to(x.device), lambda x: torch.tensor(0.0).to(x.dtype).to(x.device), lambda x: torch.tensor(0.0).to(x.dtype).to(x.device), lambda x: torch.tensor(0.0).to(x.dtype).to(x.device)
        else:
            return lambda x: self.qkv_lora1_style(x) + self.qkv_lora1_content(x), \
                   lambda x: self.qkv_lora2_style(x) + self.qkv_lora2_content(x), \
                   lambda x: self.proj_lora1_style(x) + self.proj_lora1_content(x), \
                   lambda x: self.proj_lora2_style(x) + self.proj_lora2_content(x)

    def forward(self, attn, img, txt, vec, pe, **attention_kwargs):

        qkv_lora1, qkv_lora2, proj_lora1, proj_lora2 = self.get_lora_params(self.mode)
        ...
        img_qkv = attn.img_attn.qkv(img_modulated) + qkv_lora1(img_modulated) * self.lora_weight
        ...
        txt_qkv = attn.txt_attn.qkv(txt_modulated) + qkv_lora2(txt_modulated) * self.lora_weight
        ...
        img = img + img_mod1.gate * attn.img_attn.proj(img_attn) + img_mod1.gate * proj_lora1(img_attn) * self.lora_weight
        img = img + img_mod2.gate * attn.img_mlp((1 + img_mod2.scale) * attn.img_norm2(img) + img_mod2.shift)

        # calculate the txt bloks
        txt = txt + txt_mod1.gate * attn.txt_attn.proj(txt_attn) + txt_mod1.gate * proj_lora2(txt_attn) * self.lora_weight
...
class SingleStreamPairedBlockLoraProcessor(nn.Module):
    def __init__(self, dim: int, rank: int = 4, network_alpha = None, lora_weight: float = 1):
        super().__init__()
        self.qkv_lora_content = LoRALinearLayer(dim, dim * 3, rank, network_alpha)
        self.proj_lora_content = LoRALinearLayer(15360, dim, rank, network_alpha)

        self.qkv_lora_style = LoRALinearLayer(dim, dim * 3, rank, network_alpha)
        self.proj_lora_style = LoRALinearLayer(15360, dim, rank, network_alpha)
    ...
    def get_lora_params(self, mode: str):
        if self.mode == "style":
            return self.qkv_lora_style, self.proj_lora_style
        elif self.mode == "content":
            return self.qkv_lora_content, self.proj_lora_content
        elif self.mode == "none":
            return lambda x: torch.tensor(0.0).to(x.dtype).to(x.device), lambda x: torch.tensor(0.0).to(x.dtype).to(x.device)
        else:
            return lambda x: self.qkv_lora_style(x) + self.qkv_lora_content(x), lambda x: self.proj_lora_style(x) + self.proj_lora_content(x)

    def forward(self, attn: nn.Module, x: Tensor, vec: Tensor, pe: Tensor) -> Tensor:

        qkv_lora, proj_lora = self.get_lora_params(self.mode)

        mod, _ = attn.modulation(vec)
        x_mod = (1 + mod.scale) * attn.pre_norm(x) + mod.shift
        qkv, mlp = torch.split(attn.linear1(x_mod), [3 * attn.hidden_size, attn.mlp_hidden_dim], dim=-1)
        qkv = qkv + qkv_lora(x_mod) * self.lora_weight
        ...
        output = output + proj_lora(torch.cat((attn_1, attn.mlp_act(mlp)), 2)) * self.lora_weight
