...
class XFluxPipeline:
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            inp_cond = self.prompt_embeds[prompt]
            inp_img = prepare_img(x)
            # inp_cond = prepare(t5=self.t5, clip=self.clip, img=x, prompt=prompt)
            inp_cond.update(inp_img)
            neg_inp_cond = {'txt': None, 'txt_ids': None, 'vec': None} # prepare(t5=self.t5, clip=self.clip, img=x, prompt=neg_prompt)
...
class XFluxSampler(XFluxPipeline):
    def __init__(self, clip, t5, ae, model, device, content_prompt_inps=None, offload=False):
        self.clip = clip
        self.t5 = t5
        self.ae = ae
        self.model = model
        ...
        self.ip_loaded = False
        self.offload = offload
        self.prompt_embeds = content_prompt_inps