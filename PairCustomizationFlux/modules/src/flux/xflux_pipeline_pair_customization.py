...
from modules.src.flux.sampling import denoise, denoise_controlnet, get_noise, get_schedule, prepare, unpack, prepare_img
...
class XFluxPipelinePairCustomization:
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            if self.t5 is not None and self.clip is not None:            
                ...
                assert prompt in self.content_prompt_inps or prompt in self.style_prompt_inps or prompt in self.joint_prompt_inputs, f"Prompt {prompt} not found in prompt_inps"
                
                if prompt in self.content_prompt_inps:
                    inp_cond = self.content_prompt_inps[prompt]
                elif prompt in self.style_prompt_inps:
                    inp_cond = self.style_prompt_inps[prompt]
                else:
                    inp_cond = self.joint_prompt_inputs[prompt]
                    
                img_inputs = prepare_img(x)
                inp_cond.update(img_inputs)
...
class XFluxSamplerPairCustomization(XFluxPipelinePairCustomization):
    def __init__(self, ae, model, device, clip=None, t5=None, content_prompt_inps=None, style_prompt_inps=None, joint_prompt_inputs=None, offload=False):
        self.clip = clip
        self.t5 = t5
        self.ae = ae
        self.model = model
        self.model.eval()
        self.device = device
        self.content_prompt_inps = content_prompt_inps
        self.style_prompt_inps = style_prompt_inps
        self.controlnet_loaded = False
        self.ip_loaded = False
        self.offload = offload
        self.joint_prompt_inputs = joint_prompt_inputs