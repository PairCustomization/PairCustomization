...
def load_from_repo_id(repo_id, checkpoint_name, cache_dir="/data/maxwell/models/"):
    ckpt_path = hf_hub_download(repo_id, checkpoint_name, cache_dir=cache_dir)
    sd = load_sft(ckpt_path, device='cpu')
    return sd

def load_flow_model(name: str, device: str | torch.device = "cuda", hf_download: bool = True, cache_dir="/data/maxwell/models/"):
    # Loading Flux
    print("Init model")
...
def load_flow_model2(name: str, device: str | torch.device = "cuda", hf_download: bool = True, cache_dir="/data/maxwell/models/"):
    # Loading Flux
    print("Init model")
    ckpt_path = configs[name].ckpt_path
    if (
...
def load_flow_model_quintized(name: str, device: str | torch.device = "cuda", hf_download: bool = True, cache_dir="/data/maxwell/models/"):
    # Loading Flux
    print("Init model")
    ckpt_path = configs[name].ckpt_path
    if (
        ...
        ckpt_path = hf_hub_download(configs[name].repo_id, configs[name].repo_flow, cache_dir=cache_dir)
    json_path = hf_hub_download(configs[name].repo_id, 'flux_dev_quantization_map.json', cache_dir=cache_dir)
...
def load_t5(device: str | torch.device = "cuda", max_length: int = 512, cache_dir="/data/maxwell/models/") -> HFEmbedder:
    # max length 64, 128, 256 and 512 should work (if your sequence is short enough)
    return HFEmbedder("xlabs-ai/xflux_text_encoders", max_length=max_length, torch_dtype=torch.bfloat16, cache_dir=cache_dir).to(device)

def load_clip(device: str | torch.device = "cuda", cache_dir="/data/maxwell/models/") -> HFEmbedder:
    return HFEmbedder("openai/clip-vit-large-patch14", max_length=77, torch_dtype=torch.bfloat16, cache_dir=cache_dir).to(device)


def load_ae(name: str, device: str | torch.device = "cuda", hf_download: bool = True, cache_dir="/data/maxwell/models/") -> AutoEncoder:
